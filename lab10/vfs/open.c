...
#define S_IFIMM ((I_IMMEDIATE)&S_IFMT)
#include <string.h>
...
int common_open(char path[PATH_MAX], int oflags, mode_t omode)
{
	...
	if (oflags & O_CREAT)
	{
		...
		if (r == OK)
		{
			...
			{
				printf("\nMinix-200030041_20010022 -> file created: %llu\n", vp->v_inode_nr);
			}
		}
		//-------------------Lab-9-------------------------
	...
	if (exist)
	{
		...
		if ((r = forbidden(fp, vp, bits)) == OK)
		{
			...
			//-------------------Lab-10-------------------------
			case S_IFIMM:
				/* Truncate regular file if O_TRUNC flag is set */
				if (oflags & O_TRUNC)
				{
...
static struct vnode *new_node(struct lookup *resolve, int oflags, mode_t bits)
{
	...
	if ((dirp = last_dir(&findnode, fp)) == NULL)
		return (NULL);
